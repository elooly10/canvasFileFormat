...
function resolveStyles(el, parentStyles) {
    ...
    return {
        fill: getStyle("fill", parentStyles.fill),
        stroke: getStyle("stroke", parentStyles.stroke),
        lineWidth: getNum("stroke-width", parentStyles.lineWidth),
        lineCap: getStyle("stroke-linecap", parentStyles.lineCap),
        lineJoin: getStyle("stroke-linejoin", parentStyles.lineJoin),
...
function convertElement(el, parentStyles, state) {
    ...
    if (tag === "g") {
        ...
        if (!["defs", "mask", "clippath", "symbol", "use", "title", "desc", "path", "text"].includes(tag)) {
            ...
            if (styles.stroke !== "none" && styles.lineWidth > 0)
                out.push("stroke");
        }
    }
    ...
    if ((transform || opacity !== 1)) {
        ...
        out.push("}");
        if (transform)
            out.push("reset transformations");
    }
    return out;
}
function emitStyleChanges(out, oldS, newS) {
    if (newS.fill !== oldS.fill) {
        if (newS.fill !== "none")
            out.push(`fill style ${newS.fill}`);
    }
    if (newS.stroke !== oldS.stroke) {
        if (newS.stroke !== "none")
            out.push(`stroke style ${newS.stroke}`);
    }
