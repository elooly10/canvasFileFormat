...
type command<T extends readonly (keyof ArgTypes)[]> = {
    ...
    handler: (ctx: canvasContext, args: { [K in keyof T]: T[K] extends keyof ArgTypes ? ArgTypes[T[K]] : never }) => void;
};

function createCommands<const T extends Record<string, readonly (keyof ArgTypes)[]>>(
    commands: { [K in keyof T]: command<T[K]> }
) {
    return commands;
}
...
export default createCommands({
    ...
    'line join': {
        ...
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.lineJoin = args[0] as CanvasLineJoin;
        }
    },
    'miter limit': {
        args: ['point'],
    ...
    'text align': {
        ...
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.textAlign = args[0] as CanvasTextAlign;
        }
    },
    'text baseline': {
        args: ['text'],
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.textBaseline = args[0] as CanvasTextBaseline;
        }
    },
    'text direction': {
        args: ['text'],
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.direction = args[0] as CanvasDirection;
        }
    },
    'font': {
        args: ['textArray'],
    ...
    'font kerning': {
        ...
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.fontKerning = args[0] as CanvasFontKerning;
        }
    },
    'font stretch': {
        args: ['text'],
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.fontStretch = args[0] as CanvasFontStretch;
        }
    },
    'font caps': {
        args: ['text'],
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.fontVariantCaps = args[0] as CanvasFontVariantCaps;
        }
    },
    'fill linear': {
    ...
    'composite': {
        ...
        avaliblty: 'basic',
        handler: (ctx, args) => {
            ctx.globalCompositeOperation = args[0] as GlobalCompositeOperation;
        }
    },
    'filter': {
        args: ['textArray'],
